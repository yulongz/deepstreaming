 ...
 * @date 17-12-4
 */
public class StreamVisitorBinaryDemo {
    ...
    public static void main(String[] args) {
        try(Ignite ignite = Ignition.start("default-config.xml")) {
            ...
            try(IgniteCache<String, Double> marketCache = ignite.getOrCreateCache(marketCfg);
                IgniteCache instCache = ignite.getOrCreateCache(instCfg)){
                ...
                try(IgniteDataStreamer<String, Double> streamer = ignite.dataStreamer("marketTicks")){
                    streamer.receiver(new StreamVisitor<String, Double>() {
                        ...
                        public void apply(IgniteCache<String, Double> cache, Map.Entry<String, Double> entry) {
                            ...
                            IgniteCache<String, BinaryObject> binaryCache = ignite.cache("instCache").withKeepBinary();

                            BinaryObject instrument = binaryCache.get(symbol);

                            BinaryObjectBuilder builder;

                            if (instrument == null){
                                builder = ignite.binary().builder(Instrument.class.getName());
                                builder.setField("symbol", symbol);
                            } else {
                                builder = instrument.toBuilder();
                            }

                            Double open = builder.getField("open");

                            if (open == null || open == 0) {
                                builder.setField("open", price);
                            }

                            builder.setField("latest", price);

                            instrument = builder.build();

                            binaryCache.put(symbol, instrument);
                        }
                    });
