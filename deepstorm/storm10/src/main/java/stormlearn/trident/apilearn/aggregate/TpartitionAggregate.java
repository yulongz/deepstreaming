...
public class TpartitionAggregate {
	public static StormTopology getStormTopology() {
		// declare TridentTopology
		TridentTopology topology = new TridentTopology();
		Stream streamSpout = topology.newStream("spout", new RandomBaseRichSpout()).parallelismHint(4);
		Stream streambolt = streamSpout
				// .aggregate(new Fields("sentence"), new
				// CountCombinerAggregator(), new Fields("count"))
				.partitionAggregate(new Fields("sentence"), new CountReduceAggregator(), new Fields("count"))
