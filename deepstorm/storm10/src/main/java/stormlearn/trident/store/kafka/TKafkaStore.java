...
package stormlearn.trident.store.kafka;
...
public class TKafkaStore {
	...
	public static StormTopology getStormTopology() {
		...
		// "stormlearn.trident.store.kafka.ModParititoner");
		// kafka version 0.8 not support custorm partitioner.
		// kafka version 0.9 or later support custorm partitioner.

		// TridentKafkaStateFactory
		TridentKafkaStateFactory stateFactory = new TridentKafkaStateFactory().withProducerProperties(props)
				.withKafkaTopicSelector(new DefaultTopicSelector("testp2"))
				.withTridentTupleToKafkaMapper(new FieldNameBasedTupleToKafkaMapper<String, String>("key", "value"));

		// partitionPersist
		streamSpout
				.partitionPersist(stateFactory, streamSpout.getOutputFields(), new TridentKafkaUpdater(), new Fields())
				.parallelismHint(1);
